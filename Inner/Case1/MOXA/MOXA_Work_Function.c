					...
					else if(MOXA.Fuc==0x06)
					{
						...
						{						
							...
							while(math_d)
							{
								switch(math_c)
								{	
                                    case 0x06://強制把目前安時數改成reset安時數，可用於修正負安時
                                            Coulomb_Data.x2=math_d; //將寫入值給reset電壓點
                                            math_e=Coulomb_Data.x2;
                                            Unlock_Coulomb();
                                            Write_Coulomb_Data(0x0A,Coulomb_Data.x2);//將reset電壓改為設定值
                                            index=&math_e; //目前電池容量
                                            math_b=2;
                                        break;
                                    case 0x07://將目前安時數改成設定值
